...
Use `ca learn` to store lessons; never edit `.claude/lessons/index.jsonl` directly.
Classify the item as a lesson, solution, pattern, or preference and connect superseded
or related items when supported. Show the proposed lesson and obtain the user's explicit
confirmation before storing any lesson; after confirmation, use `ca learn --yes`. Skip
generic advice, one-off facts, and conclusions already encoded in tests, lint rules, or
documentation.
